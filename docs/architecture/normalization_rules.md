...
| 10 | Duplicate observations             | `city_id` + `observed_at`                              | Composite key. When city/timestamp duplicates exist, keep the record with the higher `pipeline_run_id` (int) and drop the others.                                                  |
...
`transform-input-output-contract.md`. The gold table should still declare `NOT NULL` on `city_id`, `observed_at`, `lat`, `lon`, `aqi`, 
